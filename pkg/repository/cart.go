...
func (i *cartRepository) CreateNewCart(cart_id int) (int, error) {
	var id int

	err := i.DB.Exec(`INSERT INTO carts (user_id)VALUES ($1)`, cart_id).Error
	if err != nil {
		return 0, err
	}
	if err := i.DB.Raw(`SELECT id FROM carts WHERE user_id=?`, cart_id).Scan(&id).Error; err != nil {
		return 0, err
	}
	return id, nil
}
...
func (ad *cartRepository) GetAddresses(id int) ([]models.Address, error) {
	...
	if err := ad.DB.Raw("SELECT * FROM addresses WHERE users_id=$1", id).Scan(&addresses).Error; err != nil {
